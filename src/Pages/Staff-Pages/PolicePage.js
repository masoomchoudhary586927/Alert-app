...
// Constants for status values to avoid magic strings
const STATUS = {
  PENDING: 'pending',
  ACCEPTED: 'accepted',
  REJECTED: 'rejected',
...
const PoliceDashboard = () => {
    ...
    const fetchComplaints = async () => {
      try {
        let complaintsQuery = query(
          collection(db, 'policeComplaints'),
  ...
  const handleStatusUpdate = useCallback(async (id, status) => {
    try {
      const complaintRef = doc(db, 'policeComplaints', id);
    ...
    <div className="police-dashboard">
      <header className="dashboard-header">
        <div className="header-content">
          <h1>Police Complaints Dashboard</h1>
      ...
      <main className="dashboard-content">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>There are currently no {filter !== 'all' ? filter : ''} complaints</p>
          </div>
        ) : (
