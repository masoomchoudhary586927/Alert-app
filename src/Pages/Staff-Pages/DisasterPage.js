...
const DisasterDashboard = () => {
    ...
    const fetchComplaints = async () => {
      try {
        let complaintsQuery = query(
          collection(db, 'disasterComplaints'),
  ...
  const handleStatusUpdate = useCallback(async (id, status) => {
    try {
      const complaintRef = doc(db, 'disasterComplaints', id);
  ...
  const renderComplaintCard = (complaint) => (
    <div key={complaint.id} className={`complaint-card ${complaint.priority === 'high' ? 'priority-high' : ''}`}>
      <div className="complaint-header">
        <div>
          <h3>DR-{complaint.id.slice(0, 8).toUpperCase()}</h3>
          {complaint.timestamp && (
            <small>
      ...
      <div className="complaint-body">
        {complaint.disasterType && (
          <div className="complaint-field">
            <label>Disaster Type</label>
            <p className="disaster-type">{complaint.disasterType}</p>
          </div>
        )}
        ...
        <div className="complaint-details-grid">
          ...
          {complaint.affectedPeople && (
            <div className="complaint-field">
              ...
              <p>{complaint.affectedPeople} people</p>
            </div>
          )}
        </div>
        ...
        {complaint.location && (
          <div className="complaint-field">
            ...
            {complaint.location.service && (
              <p><strong>Service Needed:</strong> {complaint.location.service}</p>
            )}
          </div>
        )}
          ...
          <div className="complaint-field">
            ...
            <p className={`severity-${complaint.severity.toLowerCase()}`}>
              ...
              {complaint.severity.toLowerCase() === 'critical' && (
                <span className="blinking-alert"> ⚠️</span>
              )}
            </p>
          </div>
        )}
    ...
    <div className="disaster-dashboard">
      <header className="dashboard-header">
        ...
        <div className="filters">
          ...
          <button 
            className={filter === STATUS.CRITICAL ? 'active' : ''}
      ...
      <main className="dashboard-content">
        {loading ? (
          ...
          <div className="empty-state">
            <img src="/no-disasters.svg" alt="No disasters" />
            <h3>No disaster reports found</h3>
            <p>There are currently no {filter !== 'all' ? filter.replace('_', ' ') : ''} disaster reports</p>
          </div>
        ) : (
